...
class SumoLogicConnector(BaseConnector):
    ...
    def _connect(self):
        ...
        try:
            if hasattr(self, "get_phantom_home"):
                self._sumo = sumologic.SumoLogic(
                    access_id,
                    access_key,
                    endpoint=api_endpoint,
                    cookieFile=self.get_phantom_home() + "/apps/sumologic_8e235e70-57eb-4292-9b7c-6cc44847d837/cookies.txt",
                )
            else:
                self._sumo = sumologic.SumoLogic(
                    access_id,
    ...
    def _run_query(self, param):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Could not create the job with {search_string} {from_time} {to_time}", e)
        ...
        if status["state"] == "DONE GATHERING RESULTS":
            try:
                if resp_type == "messages":
                    ...
                    action_result.set_summary({"total_objects": len(response["messages"]), "search_id": search_job["id"]})

                elif resp_type == "records":
                    response = self._sumo.search_job_records(search_job, limit=limit)

                    action_result.set_summary({"total_objects": len(response["records"]), "search_id": search_job["id"]})
            except Exception:
    ...
    def _on_poll(self, param):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Failed to start job search: {e!s}")
        ...
        if status["state"] == "DONE GATHERING RESULTS":
            try:
                if job_type == "messages":
                    ...
                    return action_result.set_status(phantom.APP_ERROR, f"Invalid job type: {job_type}")
            ...
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR, f"Error occurred while fetching the search job details. Error: {e!s}")
